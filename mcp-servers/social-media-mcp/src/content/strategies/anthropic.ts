...
const logger = createComponentLogger('AnthropicStrategy');

/**
 * Anthropic content generation strategy
 * 
 * This strategy uses the Anthropic API to generate content.
 */
export class AnthropicStrategy implements ContentGenerationStrategy {
  ...
  constructor() {
    ...
    logger.info('Anthropic strategy initialized', { model: this.model });
  }
    ...
    try {
      ...
      const result = await rateLimitManager.executeRequest({
        ...
        execute: async () => {
          ...
          const systemPrompt = this.getSystemPrompt(platform, intent.contentType);
          const userPrompt = this.createPrompt(intent, research, platform);
...
export default new AnthropicStrategy();
